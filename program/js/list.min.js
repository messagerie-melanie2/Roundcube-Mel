...
1:0);this.init_header();this.frame=this.list.parentNode;this.keyboard&&(rcube_event.add_listener({event:"keydown",object:this,method:"key_press"}),$(this.list).attr("tabindex","0").on("focus",function(e){a.focus(e)}))}this.parent_focus&&(this.list.parentNode.onclick=function(e){a.focus()});rcmail.triggerEvent("initlist",{obj:this.list});return this},init_row:function(a){a.uid=this.get_row_uid(a);if(a&&a.uid){var b=this,c=a.uid;this.rows[c]={uid:c,id:a.id,obj:a};$(a).data("uid",c).mousedown(function(e){return b.drag_row(e,
this.uid)}).mouseup(function(e){return 1!=e.which||b.drag_active||$(e.currentTarget).is(".ui-droppable-active")?!0:b.click_row(e,this.uid)});if((bw.ie||bw.edge)&&bw.pointer)$(a).on("pointerdown",function(e){if("touch"==e.pointerType)return b.touch_start_time=(new Date).getTime(),!1}).on("pointerup",function(e){if("touch"==e.pointerType&&(new Date).getTime()-b.touch_start_time<=b.touch_event_time)return b.drag_row(e,this.uid),b.click_row(e,this.uid)});else bw.touch&&a.addEventListener&&(a.addEventListener("touchstart",
function(e){1==e.touches.length&&(b.touchmoved=!1,b.drag_row(rcube_event.touchevent(e.touches[0]),this.uid),b.touch_start_time=(new Date).getTime())},!1),a.addEventListener("touchend",function(e){if(1==e.changedTouches.length){var f=(new Date).getTime()-b.touch_start_time;!b.touchmoved&&f<=b.touch_event_time&&!b.click_row(rcube_event.touchevent(e.changedTouches[0]),this.uid)&&e.preventDefault()}},!1),a.addEventListener("touchmove",function(e){1==e.changedTouches.length&&(b.touchmoved=!0,b.drag_active&&
e.preventDefault())},!1));if(this.aria_listbox){var d="l:"+a.id;$(a).attr("role","option").attr("aria-labelledby",d).find(this.col_tagname()).eq(this.subject_column()).attr("id",d)}document.all&&(a.onselectstart=function(){return!1});this.row_init(this.rows[c]);this.triggerEvent("initrow",this.rows[c]);return!0}},init_header:function(){if(this.thead){this.colcount=0;this.fixed_header?($(this.list.tHead).replaceWith($(this.fixed_header).find("thead").clone()),$(this.list.tHead).find("th,td").attr("style",
"").find("a").attr("tabindex","-1")):!bw.touch&&0<=this.list.className.indexOf("fixedheader")&&this.init_fixed_header();var a,b=this;if(this.column_movable&&this.thead&&this.thead.rows)for(a=0;a<this.thead.rows[0].cells.length;a++)if(this.column_fixed!=a){var c=this.thead.rows[0].cells[a];c.onmousedown=function(d){return b.drag_column(d,this)};this.colcount++}}},init_fixed_header:function(){var a=$(this.list.tHead).clone();if(this.fixed_header)$(this.fixed_header).find("thead").replaceWith(a);else{this.fixed_header=
$("<table>").attr("class",this.list.className+" fixedcopy").attr("role","presentation").css({position:"fixed"}).append(a).append("<tbody></tbody>");$(this.list).before(this.fixed_header);var b=this;$(window).resize(function(){b.resize()});$(window).scroll(function(){var c=$(window);b.fixed_header.css({marginLeft:-c.scrollLeft()+"px",marginTop:-c.scrollTop()+"px"})})}$(this.list.tHead).find("a.sortcol").attr("tabindex","-1");a.find("a.sortcol").attr("tabindex","0");this.thead=a.get(0);this.resize()},
...
this.tbody).remove();this.rows={};this.rowcount=0;this.last_selected=null;a&&this.clear_selection();this.frame&&(this.frame.scrollTop=0);this.resize()},remove_row:function(a,b){var c=this,d=this.rows[a]?this.rows[a].obj:null;d&&(d.style.display="none",b&&this.select_next(a),delete this.rows[a],this.rowcount--,clearTimeout(this.resize_timeout),this.resize_timeout=setTimeout(function(){c.resize()},50))},insert_row:function(a,b){var c=this,d=this.tbody;if(void 0===a.nodeName){var e,f,h=document.createElement(this.row_tagname());
a.id&&(h.id=a.id);a.uid&&(h.uid=a.uid);a.className&&(h.className=a.className);a.style&&$.extend(h.style,a.style);for(e=0;a.cols&&e<a.cols.length;e++){var g=a.cols[e];var k=g.dom;if(!k)for(f in k=document.createElement(this.col_tagname()),g.className&&(k.className=g.className),g.innerHTML&&(k.innerHTML=g.innerHTML),g.events)k["on"+f]=g.events[f];h.appendChild(k)}a=h}this.checkbox_selection&&this.insert_checkbox(a);b&&d.childNodes.length?d.insertBefore(a,"object"==typeof b&&b.parentNode==d?b:d.firstChild):
...
e.type="checkbox";e.tabIndex=-1;e.onchange=function(f){c.select_row(a.uid,b||CONTROL_KEY,!0);f.stopPropagation();b=null};e.onmousedown=function(f){b=rcube_event.get_modifier(f)};d.className="selection";d.onclick=function(f){$(f.target).is("input")||(b=rcube_event.get_modifier(f),$(e).prop("checked",!e.checked).change());f.stopPropagation()};d.appendChild(e);a.insertBefore(d,a.firstChild)},enable_checkbox_selection:function(){this.checkbox_selection=!0;var a,b,c=this.row_tagname().toUpperCase(),d=
...
this.draglayer=$("<div>").attr("id","rcmdraglayer").css({position:"absolute",display:"none","z-index":2E3}).appendTo(document.body);$(this.row_tagname()+".selected",this.tbody).each(function(){var e=d.get_row_uid(this),f=d.rows[e];if(f&&!(-1<$.inArray(e,c))&&(c.push(e),f.has_children&&!f.expanded&&$.each(d.row_children(e),function(){-1<$.inArray(this,c)||c.push(this)}),11<c.length))return!1});$.each(c,function(e,f){if(10<e)return d.draglayer.append($("<div>").text("...")),!1;var h=d.subject_column();
$("> "+d.col_tagname(),d.rows[f].obj).each(function(g,k){if(0>h||0<=h&&h==g)if(k=$(k).clone(),$(k).find(".skip-on-drag").remove(),g=k.text())return g=$.trim(g),g=50<g.length?g.substring(0,50)+"...":g,d.draglayer.append($("<div>").text(g)),!1})});this.draglayer.show();this.drag_active=!0;this.triggerEvent("dragstart")}this.drag_active&&this.draglayer&&(b=rcube_event.get_mouse_pos(a),this.draglayer.css({left:b.x+20+"px",top:b.y-5+(bw.ie?document.documentElement.scrollTop:0)+"px"}),this.triggerEvent("dragmove",
