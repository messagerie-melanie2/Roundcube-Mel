...
function GoogieSpell(A,B,C){var n=this,x=rcmail.get_cookie("language");GOOGIE_CUR_LANG=null!=x?x:GOOGIE_DEFAULT_LANG;this.array_keys=function(a){var b=[],c;for(c in a)b.push([c]);return b};this.img_dir=A;this.server_url=B;this.lang_to_word=this.org_lang_to_word={da:"Dansk",de:"Deutsch",en:"English",es:"Espa\u00f1ol",fr:"Fran\u00e7ais",it:"Italiano",nl:"Nederlands",pl:"Polski",pt:"Portugu\u00eas",ru:"\u0420\u0443\u0441\u0441\u043a\u0438\u0439",fi:"Suomi",sv:"Svenska"};this.langlist_codes=this.array_keys(this.lang_to_word);
...
try{netscape.security.PrivilegeManager.enablePrivilege("UniversalBrowserRead")}catch(c){}this.main_controller&&$(this.spell_span).off("click");this.orginal_text=b.val()}};this.parseResult=function(a){var b=/\w+="(\d+|true)"/g,c=/\t/g;a=a.match(/<c[^>]*>[^<]*<\/c>/g);var d=[];if(null==a)return d;for(var e=0,f=a.length;e<f;e++){var k=[];this.errorFound();k.attrs=[];for(var g,h,p=a[e].match(b),l=0;l<p.length;l++)g=p[l].split(/=/),h=g[1].replace(/"/g,""),k.attrs[g[0]]="true"!=h?parseInt(h):h;k.suggestions=
[];g=a[e].replace(/<[^>]*>/g,"").split(c);for(h=0;h<g.length;h++)""!=g[h]&&k.suggestions.push(g[h]);d.push(k)}return d};this.processData=function(a){this.results=this.parseResult(a);this.results.length&&(this.showErrorsInIframe(),this.resumeEditingState())};this.createErrorWindow=function(){this.error_window=document.createElement("div");$(this.error_window).addClass("googie_window popupmenu").attr("googie_action_btn","1")};this.isErrorWindowShown=function(){return $(this.error_window).is(":visible")};
...
!0);this.results[a].attrs.l=c.length;this.isDefined(b.old_value)||this.saveOldValue(b,e);this.errorFixed()};this.ignoreError=function(a,b){$(a).removeAttr("class").css("color","").off();this.hideErrorWindow()};this.showErrorWindow=function(a,b){this.show_menu_observer&&this.show_menu_observer(this);var c=this,d=$(a).offset(),e=document.createElement("table"),f=document.createElement("tbody");$(this.error_window).html("");$(e).addClass("googie_list").attr("googie_action_btn","1");for(var k=!1,g=0;g<
this.custom_menu_builder.length;g++){var h=this.custom_menu_builder[g];if(h[0](this.results[b])){k=h[1](this,f,a);break}}if(!k){k=this.results[b].suggestions;var p=this.results[b].attrs.o;g=this.results[b].attrs.l;if(this.has_dictionary&&!$(a).attr("is_corrected")){h=document.createElement("tr");var l=document.createElement("td");var m=document.createElement("span");$(m).text(this.lang_learn_word);$(l).attr("googie_action_btn","1").css("cursor","default").mouseover(c.item_onmouseover).mouseout(c.item_onmouseout).click(function(q){c.learnWord(a,
b);c.ignoreError(a,b)});l.appendChild(m);h.appendChild(l);f.appendChild(h)}var u=0;for(g=k.length;u<g;u++)h=document.createElement("tr"),l=document.createElement("td"),m=document.createElement("span"),$(m).html(k[u]),$(l).mouseover(this.item_onmouseover).mouseout(this.item_onmouseout).click(function(q){c.correctError(b,a,q.target.firstChild)}),l.appendChild(m),h.appendChild(l),f.appendChild(h);if(a.is_changed&&a.innerHTML!=a.old_value){var v=a.old_value;k=document.createElement("tr");g=document.createElement("td");
h=document.createElement("span");$(h).addClass("googie_list_revert").html(this.lang_revert+" "+v);$(g).mouseover(this.item_onmouseover).mouseout(this.item_onmouseout).click(function(q){c.updateOrginalText(p,a.innerHTML,v,b);$(a).removeAttr("is_corrected").css("color","#b91414").html(v);c.hideErrorWindow()});g.appendChild(h);k.appendChild(g);f.appendChild(k)}k=document.createElement("tr");g=document.createElement("td");var r=document.createElement("input");h=document.createElement("img");l=document.createElement("form");
m=function(){""!=r.value&&(c.isDefined(a.old_value)||c.saveOldValue(a,a.innerHTML),c.updateOrginalText(p,a.innerHTML,r.value,b),$(a).attr("is_corrected",!0).css("color","green").text(r.value),c.hideErrorWindow());return!1};$(r).width(120).css({margin:0,padding:0}).val($(a).text()).attr("googie_action_btn","1");$(g).css("cursor","default").attr("googie_action_btn","1");$(h).attr("src",this.img_dir+"ok.gif").width(32).height(16).css({cursor:"pointer","margin-left":"2px","margin-right":"2px"}).click(m);
$(l).attr("googie_action_btn","1").css({margin:0,padding:0,cursor:"default","white-space":"nowrap"}).submit(m);l.appendChild(r);l.appendChild(h);g.appendChild(l);k.appendChild(g);f.appendChild(k);0<this.extra_menu_items.length&&f.appendChild(this.createListSeparator());var w=function(q){if(q<c.extra_menu_items.length){var t=c.extra_menu_items[q];if(!t[2]||t[2](a,c)){var y=document.createElement("tr"),z=document.createElement("td");$(z).html(t[0]).mouseover(c.item_onmouseover).mouseout(c.item_onmouseout).click(function(){return t[1](a,
c)});y.appendChild(z);f.appendChild(y)}w(q+1)}};w(0);w=null;this.use_close_btn&&f.appendChild(this.createCloseButton(this.hideErrorWindow))}e.appendChild(f);this.error_window.appendChild(e);g=$(this.error_window).height();e=$(this.error_window).width();h=$(document).height();k=$(document).width();g=d.top+g+20<h?d.top+20:d.top-g;d=d.left+e<k?d.left:d.left-e;$(this.error_window).css({top:g+"px",left:d+"px"}).show();document.all&&!window.opera&&(this.error_window_iframe||(d=$("<iframe>").css({position:"absolute",
...
[];for(i=0;i<this.langlist_codes.length;i++){var d=document.createElement("tr");var e=document.createElement("td");var f=document.createElement("span");$(f).text(this.lang_to_word[this.langlist_codes[i]]);this.lang_elms.push(e);$(e).attr("googieId",this.langlist_codes[i]).click(function(k){c.deHighlightCurSel();c.setCurrentLanguage($(this).attr("googieId"));null!=c.lang_state_observer&&c.lang_state_observer();c.highlightCurSel();c.hideLangWindow()}).mouseover(function(k){"googie_list_selected"!=this.className&&
(this.className="googie_list_onhover")}).mouseout(function(k){"googie_list_selected"!=this.className&&(this.className="googie_list_onout")});e.appendChild(f);d.appendChild(e);b.appendChild(d)}this.use_close_btn&&b.appendChild(this.createCloseButton(function(){c.hideLangWindow.apply(c)}));this.highlightCurSel();a.appendChild(b);this.language_window.appendChild(a)};this.isLangWindowShown=function(){return $(this.language_window).is(":visible")};this.hideLangWindow=function(){$(this.language_window).hide();
...
"change_lang.gif",alt:"Change language",googie_action_btn:"1"}),b=document.createElement("span");n=this;$(b).addClass("googie_lang_3d_on").append(a).click(function(c){c="img"==this.tagName.toLowerCase()?this.parentNode:this;$(c).hasClass("googie_lang_3d_click")?(c.className="googie_lang_3d_on",n.hideLangWindow()):(c.className="googie_lang_3d_click",n.showLangWindow(c))});return b};this.createSpellDiv=function(){var a=document.createElement("span");$(a).addClass("googie_check_spelling_link").text(this.lang_chck_spell);
...
this.all_errors_fixed_observer&&this.cnt_errors_fixed==this.cnt_errors&&(this.hideErrorWindow(),this.all_errors_fixed_observer())};this.errorFound=function(){this.cnt_errors++};this.createCloseButton=function(a){return this.createButton(this.lang_close,"googie_list_close",a)};this.createButton=function(a,b,c){var d=document.createElement("tr"),e=document.createElement("td");if(b){var f=document.createElement("span");$(f).addClass(b).html(a)}else f=document.createTextNode(a);$(e).click(c).mouseover(this.item_onmouseover).mouseout(this.item_onmouseout);
e.appendChild(f);d.appendChild(e);return d};this.removeIndicator=function(a){window.rcmail&&rcmail.set_busy(!1,null,this.rc_msg_id)};this.appendIndicator=function(a){window.rcmail&&(this.rc_msg_id=rcmail.set_busy(!0,"checking"))};this.createFocusLink=function(a){var b=document.createElement("a");$(b).attr({href:"javascript:;",name:a});return b};this.item_onmouseover=function(a){"googie_list_revert"!=this.className&&"googie_list_close"!=this.className?this.className="googie_list_onhover":this.parentNode.className=
