 ...
 */
function rcube_init_settings_tabs(){var a;var b=$("#tabsbar");var c=$("span:last",b),d="#settingstabpreferences",e=window.rcmail&&rcmail.env.action?rcmail.env.action:null;c&&"settingstababout"!=c.attr("id")&&(a=$("#settingstababout"))&&(b=a.clone(!0),a.remove(),c.after(b));e&&(d="#settingstab"+(0<e.indexOf("identity")?"identities":e.replace(/\./g,"")));$(d).addClass("tablink-selected");$("a",d).removeAttr("onclick").click(function(){return!1})}
function rcube_init_tabs(a,b){var c=$("#"+a),d=c.children("fieldset");if(d.length){b=b?b:0;d.each(function(f){f!=b&&$(this).hide()});var e=$("<div>").addClass("tabsbar").appendTo(c);d.each(function(f){var g=$(this),h=g.children("legend");var l=$("<a>").text(h.text()).attr("href","#");var k=$("<span>").attr({id:"tab"+f,"class":"tablink"}).click(function(){rcube_show_tab(a,f);return!1});h.remove();g.addClass("tabbed");f==b&&k.addClass("tablink-selected");k.append(l).appendTo(e)})}}
function rcube_show_tab(a,b){$("#"+a).children("fieldset").each(function(c){$(this)[b==c?"show":"hide"]();$("#tab"+c).toggleClass("tablink-selected",c==b)})}
...
a.command&&1==rcmail.env.reply_all_mode?(a=rcmail.gettext(a.status?"replylist":"replyall"),$("a.button.replyAll").attr("title",a)):"compose-encrypted"==a.command&&$("#messagetoolbar > a.encrypt").show()},folder_search_init:function(a){$(".boxtitle a.search",a).click(function(b){var c=$(".boxtitle",a),d=$(".listsearchbox",a),e=d.is(":visible")?-1:1,f=24+($("select",d).length?24:0);d.slideToggle({duration:160,progress:function(g,h){0>e&&(h=1-h);$(".boxlistcontent",a).css("top",c.outerHeight()+f*h+"px")},
complete:function(){d.toggleClass("expanded");d.is(":visible")?d.find("input[type=text]").focus():$("a.reset",d).click()}});return!1})}};
