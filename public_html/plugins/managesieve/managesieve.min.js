...
a=rcmail.filtersets_list.rowcount,rcmail.enable_command("plugin.managesieve-set",!0),rcmail.enable_command("plugin.managesieve-setact","plugin.managesieve-setget",0<a),rcmail.enable_command("plugin.managesieve-setdel",1<a),rcmail.enable_command("plugin.managesieve-seteditraw",0<a&&rcmail.env.raw_sieve_editor),$("tr",rcmail.gui_objects.filtersetslist).each(function(b,c){rcmail.managesieve_fixdragend(c)})));rcmail.gui_objects.sieveform&&rcmail.env.rule_disabled&&$("#disabled").attr("checked",!0)});
...
rcube_webmail.prototype.managesieve_updatelist=function(a,b){this.set_busy(!0);switch(a){case "del":var c=b.id;a=this.filters_list;a.remove_row(this.managesieve_rowid(b.id));this.show_contentframe(!1);this.reset_filters_list();$("tr",this.filters_list.list).each(function(){if("none"==this.style.display)$(this).detach();else{var l=this.id.substr(6);$(this).off();l>c&&(this.uid=l-1,$(this).attr("id","rcmrow"+this.uid))}});a.init();break;case "update":var d=$("#rcmrow"+this.managesieve_rowid(b.id));
...
b.list[d];var f=document.createElement("TR");var k=document.createElement("TD");$(k).text(e.name);k.className="name";f.id="rcmrow"+e.id;e["class"]&&(f.className=e["class"]);f.appendChild(k);a.insert_row(f)}b.set?a.highlight_row(b.set):this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);break;case "setact":c=this.managesieve_setid(b.name);d=$("#rcmrow"+c);b.active?(b.all&&$("tr",this.gui_objects.filtersetslist).addClass("disabled"),d.removeClass("disabled")):d.addClass("disabled");
...
function rule_header_select(a){var b=document.getElementById("header"+a),c=document.getElementById("rule_size"+a),d=document.getElementById("rule_message"+a),e=document.getElementById("rule_op"+a),f=document.getElementById("custom_header"+a+"_list"),k=document.getElementById("custom_var"+a+"_list"),l=document.getElementById("rule_mod"+a),m=document.getElementById("rule_trans"+a),q=document.getElementById("rule_comp"+a),r=document.getElementById("rule_date_part"+a),t=document.getElementById("rule_date_header_div"+
a),n=$("#rule_op"+a),g=b.value,p=[e,f,k,l,m,q,c];"size"==g?(d&&p.push(d),$.each(p,function(){this.style.display="none"}),c.style.display="inline"):"message"==g&&d?($.each(p,function(){this.style.display="none"}),d.style.display="inline"):(f.style.display="..."!=g?"none":"inline-block",k.style.display="string"!=g?"none":"inline-block",c.style.display="none",e.style.display="inline",q.style.display="",l.style.display="body"==g||"currentdate"==g||"date"==g||"string"==g?"none":"block",m.style.display=
"body"==g?"block":"none",d&&(d.style.display="message"==g?"block":"none"));r&&(r.style.display="currentdate"==g||"date"==g?"inline":"none");t&&(t.style.display="date"==g?"":"none");$('[value="exists"],[value="notexists"]',n).prop("disabled","string"==g);n.val()||n.val("contains");rule_op_select(e,a,g);rule_mod_select(a,g);b.style.width="..."==g?"40px":""}
...
function smart_field_init(a){var b=a.id+"_list",c=$('<span class="listarea"></span>'),d=a.value?a.value.split("\n"):[""];$("#"+b).length||($.each(d,function(e,f){c.append(smart_field_row(f,a.name,e,$(a).data("size")))}),c.attr("id",b),a=$(a),a.attr("disabled")?c.hide():a.prop("disabled",!0),a.after(c),a.hasClass("error")&&(c.addClass("error"),rcmail.managesieve_tip_register([[b,a.data("tip")]])))}
function smart_field_row(a,b,c,d){c=$('<span class="listelement"><span class="reset"></span><input type="text"></span>');a={value:a,name:b+"[]"};d&&(a.size=d);$("input",c).attr(a).keydown(function(e){var f=$(this);if(13==e.which){e=f.attr("name").replace(/\[\]$/,"");var k=(new Date).getTime();e=smart_field_row("",e,k,d);f.parent().after(e);$("input",e).focus()}else if((8==e.which||46==e.which)&&""==f.val()&&(f=f.parent(),1<f.parent().children().length))return f.prev().length?f.prev().children("input").focus():
f.next().children("input").focus(),f.remove(),!1});$('span[class="reset"]',c).click(function(){var e=$(this.parentNode);1<e.parent().children().length?e.remove():$("input",e).val("").focus()});return c}function smart_field_reset(a,b){b=b.length?b:[""];area=$("#"+(a.id+"_list"));area.empty();$.each(b,function(c,d){area.append(smart_field_row(d,a.name,c,$(a).data("size")))})}
rcube_webmail.prototype.managesieve_tip_register=function(a){var b,c=parent.rcmail,d=c?parent.rcmail.env.ms_tip_layer:rcmail.env.ms_tip_layer;for(b in a)$("#"+a[b][0]).data("tip",a[b][1]).mouseleave(function(e){d.hide()}).mouseenter(function(e){e=$(this);var f=e.offset(),k=f.left,l=f.top-12,m=e.width();c&&(f=$("mail"==rcmail.env.task?"#sievefilterform > iframe":"#filter-box",parent.document).offset(),l+=f.top,k+=f.left);d.html(e.data("tip"));l-=d.height();d.css({left:k,top:l,minWidth:m-2+"px"}).show()})};
function sieve_formattime(a,b){var c,d="",e=rcmail.env.time_format||"H:i";for(c=0;c<e.length;c++){var f=e.charAt(c);switch(f){case "a":d+=12<=a?"pm":"am";break;case "A":d+=12<=a?"PM":"AM";break;case "g":case "h":d+=("h"==f&&10>a?"0":"")+a;break;case "G":d+=a;break;case "H":d+=(10>a?"0":"")+a;break;case "i":d+=(10>b?"0":"")+b;break;case "s":d+="00";default:d+=f}}return d}
