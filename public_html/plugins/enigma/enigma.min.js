...
(window.crypto.getRandomValues||window.crypto.subtle))){var e=this.set_busy(!0,"enigma.keygenerating");c={numBits:d,userIds:a,passphrase:b};openpgp.generateKey(c).then(function(f){rcmail.http_post("plugin.enigmakeys",{_a:"import",_keys:f.privateKeyArmored,_generated:1,_passwd:b,_keyid:f.key.primaryKey.fingerprint},e)},function(f){console.error(f);rcmail.set_busy(!1,null,e);rcmail.display_message(rcmail.get_label("enigma.keygenerateerror"),"error")})}else rcmail.display_message(rcmail.get_label("enigma.keygennosupport"),
...
this,a[this])});e.text(c);this.show_popup_dialog(d,this.get_label("enigma.enterkeypasstitle"),[{text:this.get_label("save"),"class":"mainaction",click:function(g){g.stopPropagation();g=b.is_framed()?window.parent.$:$;a.password=f.val();a.password?(b.enigma_password_submit(a),g(this).remove()):f.focus()}},{text:this.get_label("cancel"),click:function(g){var h=b.is_framed()?window.parent.$:$;g.stopPropagation();h(this).remove()}}],{width:400});this.is_framed()&&parent.rcmail.message_list&&parent.rcmail.message_list.blur()}};
...
rcube_webmail.prototype.enigma_find_publickey=function(a){this.mailvelope_search_pubkeys([a],function(b){},function(b){var c=rcmail.set_busy(!0,"enigma.importwait");b={_a:"import",_keys:b};"plugin.enigmakeys"==rcmail.env.action&&(b._refresh=1);rcmail.http_post("plugin.enigmakeys",b,c)})};
